...
(defvar *auto-zoom* t)
(defvar *visualiser-text-fn*
  (lambda (sat) (format nil "~A ~,3E" (sat-name sat) (sat-r sat))))
...
(defun visualise (sim &key (earth-color (sdl:color :r 20 :g 100 :b 100))
  ...
  (let* ((time 0) (scale (* 2 +radius-earth+)) playing window (frame (pop frames)) (first-time-p t))
    (labels (
	     (sats ()
	     ...
	     (rescale ()
		   ...
		   (when first-time-p
		     (setf first-time-p nil scale *visualise-max-radius*))))
	     (window-scale ()
	       (/ (* 2 scale)
		 ...
		 (loop for i downfrom (1- (length sats)) to 0 
		    ...
		    (sdl:draw-filled-circle-* (xform-x (sat-x sat)) (xform-y (sat-y sat))
					      ...
					      :color (sat-color sat))
		    (visualise-draw-text (funcall *visualiser-text-fn* sat)
